...
static int init_game(game_t *game)
{
	...
	if (game->keys == NULL || init_sounds(game) == 84 ||
	init_window(game, video) == 84 || init_objects(game) == 84)
		return (84);
	init_keys(game);
	...
	game->current_text = 1;
	game->slot_rect = 2253;
	game->character->char_obj = create_object(game->character->
	char_obj, game);
	game->weapons = create_object(game->weapons, game);
...
int main(void)
{
	...
	if (environ[0] == NULL ||game == NULL || particules == NULL)
		return (84);
	srand(time(NULL));
